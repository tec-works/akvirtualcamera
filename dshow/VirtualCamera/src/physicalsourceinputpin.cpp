...
PhysicalSourceInputPin::PhysicalSourceInputPin(BaseFilter* pFilter, HRESULT* phr, LPCWSTR pPinName)
    : CUnknown(L"PhysicalSourceInputPin", nullptr), // Pass nullptr for outer unknown for now
      m_pFilter(pFilter),
...
STDMETHODIMP PhysicalSourceInputPin::ConnectionMediaType(AM_MEDIA_TYPE *pmt)
{
    ...
    *pmt = CreateMediaType(&m_mt); // Helper from dshowutil.h or implement locally
    if (pmt->pbFormat != nullptr && m_mt.cbFormat > 0) {
