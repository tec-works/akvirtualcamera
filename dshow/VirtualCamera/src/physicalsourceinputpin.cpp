...
STDMETHODIMP PhysicalSourceInputPin::ConnectionMediaType(AM_MEDIA_TYPE *pmt)
{
    ...
    // Use the project's lowercase 'createMediaType'
    // This function (from PlatformUtils/utils.h) likely allocates and copies.
    // We need to ensure it's used correctly according to its signature.
    // Assuming createMediaType fills the passed AM_MEDIA_TYPE structure.
    // The original dshowutil.h CreateMediaType returns a new AM_MEDIA_TYPE*.
    // This project's createMediaType seems to be void createMediaType(AM_MEDIA_TYPE* pmtDest, const AM_MEDIA_TYPE* pmtSrc);
    // So, we need to pass our destination pmt.

    // Free existing format block in pmt if any (as per CoTaskMemAlloc convention)
    ...
    AkVCam::createMediaType(pmt, &m_mt); // Corrected call

    // createMediaType should handle allocation of pbFormat and AddRef for pUnk.
    // No need to manually copy pbFormat or AddRef pUnk if createMediaType does it.
    // The original manual copy here was based on the assumption that *pmt = m_mt was a shallow copy.
    // If AkVCam::createMediaType does a deep copy including pbFormat allocation, this is simpler.
    // Let's verify the signature of AkVCam::createMediaType.
    // From PlatformUtils/utils.h: void createMediaType(AM_MEDIA_TYPE* pmtDest, const AM_MEDIA_TYPE* pmtSrc);
    // It does a deep copy.

    // if (pmt->pbFormat != nullptr && m_mt.cbFormat > 0) { // This block is now handled by createMediaType
    //     pmt->pbFormat = (BYTE*)CoTaskMemAlloc(m_mt.cbFormat);
    //     if (pmt->pbFormat == nullptr) return E_OUTOFMEMORY;
    //     CopyMemory(pmt->pbFormat, m_mt.pbFormat, m_mt.cbFormat);
    // }
    // if (m_mt.pUnk != nullptr) { // This is also handled by createMediaType
    //     m_mt.pUnk->AddRef();
    // }
    return S_OK;
}
...
STDMETHODIMP PhysicalSourceInputPin::QueryPinInfo(PIN_INFO *pInfo)
{
    ...
    // Use a helper for string copy if available, or wcscpy_s
    // #include <wchar.h> for wcscpy_s if not already available via other headers
...
STDMETHODIMP PhysicalSourceInputPin::Receive(IMediaSample *pSample)
{
    ...
    if (SUCCEEDED(hr) && pData && len > 0) {
        ...
        // This requires adding a method like NotifyPhysicalFrameReady to BaseFilter.
...
STDMETHODIMP PhysicalSourceInputPin::QueryId(LPWSTR *Id)
{
    AkLogFunction();
    if (!Id) return E_POINTER;
    *Id = (LPWSTR)CoTaskMemAlloc((m_pinName.length() + 1) * sizeof(WCHAR));
    if (!*Id) return E_OUTOFMEMORY;
    wcscpy_s(*Id, m_pinName.length() + 1, m_pinName.c_str());
    return S_OK;
}
