...
namespace AkVCam {
    ...
    class CmdParserPrivate
    {
        public:
            ...
            int listPhysicalCamerasHandler(const StringMap &flags, const StringVector &args);
...
AkVCam::CmdParser::CmdParser()
{
    ...
    this->addFlags("add-device",
                   {"--source-camera"},
                   "SOURCENAME",
                   "Use physical camera SOURCENAME as source.");
    this->addFlags("add-device",
                   {"--num-virtual-cameras"},
                   "COUNT",
                   "Number of virtual cameras to create.");
    ...
    this->addCommand("list-cameras",
                     "",
                     "List physical cameras.",
                     AKVCAM_BIND_FUNC(CmdParserPrivate::listPhysicalCamerasHandler));
}
                                        ...
                                        const StringVector &args)
{
    ...
    auto sourceCamera = this->flagValue(flags, "add-device", "--source-camera");
    auto numVirtualCamerasStr = this->flagValue(flags, "add-device", "--num-virtual-cameras");

    int numVirtualCameras = 1;
    if (!numVirtualCamerasStr.empty()) {
        char *p = nullptr;
        const char* numStr = numVirtualCamerasStr.c_str();
        numVirtualCameras = ::strtol(numStr, &p, 10);
    ...
    if (!sourceCamera.empty()) {
        // Webcam splitting logic
        std::cout << "Webcam splitting: source=" << sourceCamera
                  << ", num_virtual=" << numVirtualCameras
                  << ", description=" << args[1]
                  << (deviceId.empty() ? "" : ", id=" + deviceId)
                  << std::endl;
        // TODO: Implement actual webcam splitting IPC call
        // For now, just print a message
        for (int i = 0; i < numVirtualCameras; ++i) {
            std::string currentDeviceId = deviceId;
            if (numVirtualCameras > 1) {
                 currentDeviceId = deviceId.empty() ? "" : deviceId + "_" + std::to_string(i+1);
            }
            std::string currentDescription = args[1];
            if (numVirtualCameras > 1) {
                currentDescription = args[1] + " (Split " + std::to_string(i+1) + ")";
            }

            // This is a placeholder for the actual IPC call
            // We'll need to extend IpcBridge to handle this
            std::string createdId = this->m_ipcBridge.addDevice(currentDescription, currentDeviceId, sourceCamera);
            if (createdId.empty()) {
                std::cerr << "Failed to create virtual camera " << (i+1) << std::endl;
                // Potentially roll back previously created devices or just report error
            } else {
                if (this->m_parseable)
                    std::cout << createdId << std::endl;
                else
                    std::cout << "Virtual camera " << (i+1) << " created as " << createdId << " (Source: " << sourceCamera << ")" << std::endl;
            }
        }

    } else {
        // Existing logic for adding a single, non-sourced virtual camera
                                        ...
                                        const StringVector &args)
{
    ...
    if (!indexStr.empty()) {
        char *p_index = nullptr; // Changed variable name to avoid conflict
        const char* indexCStr_af = indexStr.c_str();
        unsigned long ulong_idx = ::strtoul(indexCStr_af, &p_index, 10);

        if (*p_index) {
            std::cerr << "Index must be an unsigned integer." << std::endl;
            return -EINVAL;
        }
        // Check if ulong_idx can be safely cast to int
        if (ulong_idx > static_cast<unsigned long>(std::numeric_limits<int>::max())) {
            std::cerr << "Index value out of range for int: " << ulong_idx << std::endl;
            return -ERANGE;
        }
        index = static_cast<int>(ulong_idx);
    }

    VideoFormat fmt(format, static_cast<int>(width), static_cast<int>(height), {fps});
                                     ...
                                     const AkVCam::StringVector &args)
{
    ...
    if (!fpsStr.empty()) {
        char *p = nullptr; // p was removed in std::stod version, re-add for ::strtod
        const char* fpsCStr = fpsStr.c_str();
        fps = ::strtod(fpsCStr, &p); // int() cast was removed as fps is double

        if (*p) { // Check if all characters were consumed by strtod
        ...
        for (auto &control: this->m_ipcBridge.controls(deviceId))
                if (control.id == key) {
                    ...
                    case ControlTypeInteger: {
                        ...
                        auto val = ::strtol(valStr, &p, 10);
                        if (*p) {
                            std::cerr << "Value at argument " << i << " (for " << key << ") must be an integer." << std::endl;
                            return -EINVAL;
                        }
                        controls[key] = val;
                        break;
                    ...
                    case ControlTypeBoolean: {
                        ...
                        if (lower_value == "0" || lower_value == "false") {
                            ...
                            std::cerr << "Value at argument " << i << " must be a boolean." << std::endl;
                            return -EINVAL;
                        }
                        break;
                    }
                    ...
                    case ControlTypeMenu: {
                        ...
                        auto val = ::strtoul(valStr_wc, &p, 10);
                        if (*p) { // Not a pure number, try as string
                            auto it = std::find(control.menu.begin(), control.menu.end(), value);
                            if (it == control.menu.end()) {
                                std::cerr << "Value at argument " << i << " (for " << key << ") is not a valid menu option." << std::endl;
                                return -EINVAL;
                            }
...
int AkVCam::CmdParserPrivate::listPhysicalCamerasHandler(const AkVCam::StringMap &flags,
                                                         const AkVCam::StringVector &args)
{
    UNUSED(flags);
    UNUSED(args);

    // TODO: Implement platform-specific camera listing
#if defined(_WIN32)
    ...
    if (FAILED(hr)) {
        std::cerr << "Failed to create SystemDeviceEnum" << std::endl;
        CoUninitialize();
        return -1;
    }
    ...
    if (FAILED(hr) || pEnum == nullptr) {
        std::cerr << "Failed to create class enumerator or no devices found" << std::endl;
        if (pDevEnum) pDevEnum->Release();
        CoUninitialize();
        return SUCCEEDED(hr) ? 0 : -1; // Return 0 if no devices found, -1 on error
    }

    std::cout << "Physical Cameras (Windows):" << std::endl;
    ...
    // Implementation for macOS using AVFoundation
    // This requires Objective-C or Swift, so we'll need a separate .mm file
    // or use a helper function defined in an Objective-C context.
    // For now, let's assume a helper function `listObjCCameras()` exists.
    // This function would be implemented in a .mm file and linked appropriately.
    extern std::vector<std::string> listObjCCameras();
    std::vector<std::string> cameras = listObjCCameras();
    std::cout << "Physical Cameras (macOS):" << std::endl;
    if (cameras.empty()) {
...
#elif defined(__linux__)
    // Implementation for Linux using V4L2
    std::cout << "Physical Cameras (Linux):" << std::endl;
    for (int i = 0; i < 64; ++i) { // Check up to 64 video devices
        std::string devicePath = "/dev/video" + std::to_string(i);
        int fd = open(devicePath.c_str(), O_RDONLY);
        if (fd == -1) {
            if (errno == ENOENT) continue; // No such file, try next
            // Potentially other errors, but we'll skip for simplicity here
            continue;
        }
        ...
        if (cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) {
            std::cout << "  - " << reinterpret_cast<char*>(cap.card) << " (" << devicePath << ")" << std::endl;
        }
    }
#else
...
void AkVCam::CmdParserPrivate::loadGenerals(Settings &settings)
{
    ...
    if (settings.contains("loglevel")) {
        ...
        auto level = ::strtol(logLevelCStr, &p, 10);

        if (*p) // If not all characters were consumed, it's a string label
...
std::vector<AkVCam::VideoFormat> AkVCam::CmdParserPrivate::readFormat(Settings &settings)
{
    ...
    for (auto &format_list: this->matrixCombine(formatMatrix)) {
        ...
        auto width = ::strtol(widthStr, &p, 10);
        // TODO: Add error check for *p if necessary, though original code didn't for these
        p = nullptr;
        const char* heightStr = format_list[2].c_str();
        auto height = ::strtol(heightStr, &p, 10);
        // TODO: Add error check for *p if necessary

        Fraction frame_rate(format_list[3]);
        VideoFormat format(pixFormat,
                           static_cast<int>(width), // Assuming width fits in int
                           static_cast<int>(height), // Assuming height fits in int
                           {frame_rate});
                                                                             ...
                                                                             const VideoFormatMatrix &availableFormats)
{
    ...
    for (auto &indexStr: formatsIndex) {
        char *p = nullptr;
        // Revert to using ::strtoul, assuming this was the state before std::stoul attempt
        const char* indexCStr_rdf = indexStr.c_str();
        auto index = ::strtoul(indexCStr_rdf, &p, 10);

        if (*p) // If conversion did not consume the whole string or failed
            continue;
